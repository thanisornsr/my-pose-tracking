...
dummy_input_batch = np.zeros((BATCH_SIZE,368,368,3),dtype = np.float32)


if USE_FP16:
    os.system("/usr/src/tensorrt/bin/trtexec --onnx=TFF_onnx_model.onnx --saveEngine=TFF_engine.trt --explicitBatch --fp16")
else:
    os.system("/usr/src/tensorrt/bin/trtexec --onnx=TFF_onnx_model.onnx --saveEngine=TFF_engine.trt --explicitBatch")
